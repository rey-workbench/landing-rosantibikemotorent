<script lang="ts">
	...
	let message = $derived(
		page.status === 404
			...
			: page.error?.message || (isEn ? 'An unexpected server error occurred.' : 'Terjadi kendala teknis pada sistem kami.')
	);
...
<section class="min-h-[85vh] flex items-center justify-center px-4 sm:px-6 pt-32 pb-20 md:pt-40 md:pb-28 bg-[#f5f5f7] relative overflow-hidden">
	<!-- Background subtle decorative glow -->
	<div class="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-96 h-96 bg-apple-blue/5 rounded-full blur-3xl pointer-events-none"></div>

	<div class="max-w-xl w-full bg-white rounded-3xl md:rounded-[2.5rem] p-8 sm:p-12 md:p-14 shadow-[0_4px_40px_rgba(0,0,0,0.04)] border border-black/5 text-center relative z-10">
		
		<!-- Status Code Tag -->
		<div class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-[#f5f5f7] border border-black/5 text-xs font-semibold tracking-wider text-[#1d1d1f] mb-8">
			<span class="w-2 h-2 rounded-full {status === 404 ? 'bg-amber-500' : 'bg-red-500'} animate-pulse"></span>
			<span>{status} &bull; {status === 404 ? (isEn ? 'PAGE NOT FOUND' : 'RUTE TERPUTUS') : (isEn ? 'SERVER ERROR' : 'KENDALA SISTEM')}</span>
		</div>
		...
		<div class="pt-8 border-t border-black/5 text-left">
			...
			<div class="grid grid-cols-2 sm:grid-cols-3 gap-2">
				...
				>
					<span class="text-xs font-semibold text-[#1d1d1f] group-hover:text-apple-blue transition-colors">Booking Online</span>
					<span class="text-[11px] text-[#86868b]">{isEn ? 'Direct reservation' : 'Pesan unit cepat'}</span>
				</a>
				<a
					href="/{lang}/faq"
					class="p-3 rounded-2xl bg-[#f5f5f7] hover:bg-[#ebebf0] transition-colors flex flex-col group border border-transparent hover:border-black/5"
				>
					<span class="text-xs font-semibold text-[#1d1d1f] group-hover:text-apple-blue transition-colors">FAQ & Bantuan</span>
					<span class="text-[11px] text-[#86868b]">{isEn ? 'Questions & rules' : 'Syarat & tarif'}</span>
				</a>
				<a
					href="/{lang}/blog"
				...
				>
					<span class="text-xs font-semibold text-[#1d1d1f] group-hover:text-apple-blue transition-colors">Blog Wisata</span>
		...
		<div class="mt-6 pt-4 text-center">
			...
			>
				...
				<span>{isEn ? 'Need immediate support? Chat Customer Service on WhatsApp' : 'Butuh bantuan segera? Chat CS Rosantibike via WhatsApp'}</span>
				&rarr;
			</a>
		</div>
